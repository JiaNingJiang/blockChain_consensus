...
func (s *CommonSuite) TestOS(c *checker.C) {
	...
	if expwin {
		c.Assert(res, checker.Equals, expwin, checker.Commentf("IsWindows is %t but path is %c", res, os.PathSeparator))
	} else {
		c.Assert(res, checker.Not(checker.Equals), expwin, checker.Commentf("IsWindows is %t but path is %c", res, os.PathSeparator))
	}
}
